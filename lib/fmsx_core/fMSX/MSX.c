...
/* Fold a VDP table offset into the VRAM this machine actually has. See
 * the comment in VDPOut() on register 2 for why this is not optional. */
#define VRAM_OFS(O) ((VRAMPages>0)? ((O)&((VRAMPages<<14)-1)) : 0)

/* Embedded BIOS images, used when there is no MSX.ROM on an SD card.
 * cbios_main_msx1_rom is C-BIOS (src/cbios_data.c, BSD-style licensed) and
 * ships with the repo; local_bios_rom is the owner's own dumped BIOS
 * (src/hotbit_bios_data.c, generated by tools/embed_rom.py and gitignored),
 * preferred when present because a real machine BIOS boots MSX-BASIC and
 * C-BIOS, being cartridge-only, does not. */
extern const unsigned char cbios_main_msx1_rom[32768];
#ifdef HAVE_LOCAL_BIOS
extern const unsigned char local_bios_rom[32768];
#define EMBEDDED_BIOS local_bios_rom
#else
#define EMBEDDED_BIOS cbios_main_msx1_rom
#endif

#ifdef HAVE_LOCAL_CART
/* Cartridges built into the firmware, for a board with no room to hold
 * one in RAM. The catalogue is generated by tools/embed_roms.py into
 * src/msx/local_cart_data.c and is gitignored, like the BIOS; which one
 * is selected lives in msx_carts.c. LoadCart() takes this path as a name
 * meaning "the one in flash" rather than a file. */
const unsigned char *msx_cart_image(void);
int msx_cart_size(void);
#define LOCAL_CART_PATH "flash:cart"
#endif
...
int StartMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  /* EmptyRAM is a fixed 16kB block that never changes size, so it is a
   * static array rather than a heap allocation. That is not tidiness: on
   * this board the DRAM comes in a few fixed regions, and only one of
   * them is big enough to hold the framebuffer, the 32kB of emulated RAM
   * and the 16kB of VRAM together. A 16kB malloc landing in the middle of
   * that region before them is the difference between booting and not.
   * See docs/MEMORY.md. */
  {
    static byte EmptyRAMBuf[0x4000];
    EmptyRAM=EmptyRAMBuf;
  }
  ...
  if(LoadROM("/sdcard/msx/bios/CMOS.ROM",sizeof(RTC),(byte *)RTC))
  { if(Verbose) printf("CMOS.ROM.."); }
  else memcpy(RTC,RTCInit,sizeof(RTC));

  /* Try loading Kanji alphabet ROM */
  if((Kanji=LoadROM("/sdcard/msx/bios/KANJI.ROM",0x20000,0)))
  { if(Verbose) printf("KANJI.ROM.."); }

  /* Try loading RS232 support ROM to slot */
  if((P=LoadROM("/sdcard/msx/bios/RS232.ROM",0x4000,0)))
  {
  ...
  if(!MODEL(MSX_MSX1)&&OPTION(MSX_MSXDOS2)&&(MemMap[3][1][2]!=EmptyRAM)&&!ROMData[2])
    if(LoadCart("/sdcard/msx/bios/MSXDOS2.ROM",2,MAP_GEN16))
  ...
  if(!MODEL(MSX_MSX1))
  {
    ...
    if((J<MAXSLOTS)&&LoadCart("/sdcard/msx/bios/PAINTER.ROM",J,0)) ++J;
  }

  /* Load FMPAC cartridge */
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
  if((J<MAXSLOTS)&&LoadCart("/sdcard/msx/bios/FMPAC.ROM",J,MAP_FMPAC)) ++J;

  /* Load Konami GameMaster2/GameMaster cartridges */
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
  if(J<MAXSLOTS)
  {
    if(LoadCart("/sdcard/msx/bios/GMASTER2.ROM",J,MAP_GMASTER2)) ++J;
    else if(LoadCart("/sdcard/msx/bios/GMASTER.ROM",J,0)) ++J;
  }
...
void TrashMSX(void)
{
  ...
  if(SaveCMOS)
  {
    ...
    if(!(F=fopen("/sdcard/msx/bios/CMOS.ROM","wb"))) SaveCMOS=0;
    else
    {
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if((Mode^NewMode)&MSX_MODEL)
  {
    ...
    switch(NewMode&MSX_MODEL)
    {
      case MSX_MSX1:
        ...
        P1=LoadROM("/sdcard/msx/bios/MSX.ROM",0x8000,0);
        PRINTRESULT(P1);
      ...
      case MSX_MSX2:
        ...
        P1=LoadROM("/sdcard/msx/bios/MSX2.ROM",0x8000,0);
        PRINTRESULT(P1);
        if(Verbose) printf("  Opening MSX2EXT.ROM...");
        P2=LoadROM("/sdcard/msx/bios/MSX2EXT.ROM",0x4000,0);
        PRINTRESULT(P2);
        if(!P1||!P2) 
        {
      ...
      case MSX_MSX2P:
        ...
        P1=LoadROM("/sdcard/msx/bios/MSX2P.ROM",0x8000,0);
        PRINTRESULT(P1);
        if(Verbose) printf("  Opening MSX2PEXT.ROM...");
        P2=LoadROM("/sdcard/msx/bios/MSX2PEXT.ROM",0x4000,0);
        PRINTRESULT(P2);
        if(!P1||!P2) 
        {
          NewMode=(NewMode&~MSX_MODEL)|(Mode&MSX_MODEL);
  ...
  /* ...unless the BIOS is being executed straight out of flash, which
   * cannot be written to. These patches only replace the cassette BIOS
   * entry points with fMSX's trap opcode, and there is no cassette on
   * this board, so skipping them costs nothing and saves a 32kB RAM copy
   * of the ROM on a machine that has none to spare. */
  if(((Mode^NewMode)&MSX_MODEL)
     && (MemMap[0][0][0]!=(byte *)EMBEDDED_BIOS))
  {
  ...
  if((Mode^NewMode)&MSX_PATCHBDOS)
  {
    ...
    P1=LoadROM("/sdcard/msx/bios/DISK.ROM",0x4000,0);
    PRINTRESULT(P1);
  ...
  /* Upstream floors an MSX1 at 4 pages (64kB). Plenty of real MSX1
   * machines shipped with 32kB or less, and on this board 32kB is the
   * difference between booting and not - see docs/MEMORY.md. */
  if((NewRAMPages<(MODEL(MSX_MSX1)? 2:8))||(NewRAMPages>256))
    NewRAMPages=MODEL(MSX_MSX1)? 2:8;
  /* An MSX1 is allowed one 16kB page here, which is what a real TMS9918
   * has, where upstream floors it at two. That is safe only because every
   * VDP table pointer is folded into the VRAM that exists - see VRAM_OFS
   * in VDPOut(). Without that clamp, 16kB crashed C-BIOS outright and
   * 32kB merely hid the same bug behind a bigger buffer. */
  if((NewVRAMPages<1)||(NewVRAMPages>(MODEL(MSX_MSX1)? 2:8)))
    NewVRAMPages=MODEL(MSX_MSX1)? 1:8;
...
byte SetScreen(void)
{
  ...
  /* Recompute table addresses, every one of them folded into the VRAM
   * this machine actually has - see the note on VRAM_OFS in VDPOut().
   * This is the copy that mattered: a screen-mode change recomputes all
   * five here, so clamping only the register writes left the pointers
   * exactly as wrong as before. */
  I=(J>6)&&(J!=MAXSCREEN+1)? 11:10;
  ChrTab  = VRAM+VRAM_OFS((int)(VDP[2]&MSK[J].R2)<<I);
  ChrGen  = VRAM+VRAM_OFS((int)(VDP[4]&MSK[J].R4)<<11);
  ColTab  = VRAM+VRAM_OFS(((int)(VDP[3]&MSK[J].R3)<<6)+((int)VDP[10]<<14));
  SprTab  = VRAM+VRAM_OFS(((int)(VDP[5]&MSK[J].R5)<<7)+((int)VDP[11]<<15));
  SprGen  = VRAM+VRAM_OFS((int)VDP[6]<<11);
...
{ 
  ...
  switch(R)  
  {
    ...
    case  1: /* Set/Reset VBlank interrupt if enabled or disabled */
             ...
             break;
    /* Every one of these is VRAM plus an offset built from a VDP
     * register, and the register can address more VRAM than this machine
     * has: the masks are sized for the 128kB an MSX2 can carry. With
     * 32kB allocated, a register 2 of 0x7F puts the name table 128kB past
     * the end of the buffer, and the renderer then reads whatever is
     * there - which on this chip was Bluetooth ROM, and a byte read from
     * instruction space is not merely wrong, it is a fault.
     *
     * Wrapping the offset into the VRAM that exists is also what the
     * hardware does: the address lines above the fitted memory are simply
     * not connected. VRAM_OFS() does that, and every table goes through
     * it. Upstream does not do this because upstream assumes you gave the
     * machine as much VRAM as its registers can address. */
    case  2: J=(ScrMode>6)&&(ScrMode!=MAXSCREEN+1)? 11:10;
             ChrTab  = VRAM+VRAM_OFS((int)(V&MSK[ScrMode].R2)<<J);
             ChrTabM = ((int)(V|~MSK[ScrMode].M2)<<J)|((1<<J)-1);
             break;
    case  3: ColTab  = VRAM+VRAM_OFS(((int)(V&MSK[ScrMode].R3)<<6)+((int)VDP[10]<<14));
             ColTabM = ((int)(V|~MSK[ScrMode].M3)<<6)|0x1C03F;
             break;
    case  4: ChrGen  = VRAM+VRAM_OFS((int)(V&MSK[ScrMode].R4)<<11);
             ChrGenM = ((int)(V|~MSK[ScrMode].M4)<<11)|0x007FF;
             break;
    case  5: SprTab  = VRAM+VRAM_OFS(((int)(V&MSK[ScrMode].R5)<<7)+((int)VDP[11]<<15));
             SprTabM = ((int)(V|~MSK[ScrMode].M5)<<7)|0x1807F;
             break;
    case  6: V&=0x3F;SprGen=VRAM+VRAM_OFS((int)V<<11);break;
    case  7: FGColor=V>>4;BGColor=V&0x0F;break;
    case 10: V&=0x07;
             ColTab=VRAM+VRAM_OFS(((int)(VDP[3]&MSK[ScrMode].R3)<<6)+((int)V<<14));
             break;
    case 11: V&=0x03;
             SprTab=VRAM+VRAM_OFS(((int)(VDP[5]&MSK[ScrMode].R5)<<7)+((int)V<<15));
             break;
...
int LoadFile(const char *FileName)
{
  ...
  if (fullPathName == NULL)
    fullPathName = malloc(1024);
  strcpy(fullPathName, fullCurrentDir);
  strcat(fullPathName, "/");
  ...
  if(hasext(FileName,".DSK")||hasext(FileName,".FDI"))
  {
    ...
    if(!ChangeDisk(0,fullPathName)) return(0);
  ...
  if(hasext(FileName,".ROM")||hasext(FileName,".MX1")||hasext(FileName,".MX2"))
    return(!!LoadCart(fullPathName,0,ROMGUESS(0)|ROMTYPE(0)));

  /* Try loading as a tape */
  if(hasext(FileName,".CAS")) return(!!ChangeTape(fullPathName));
  /* Try loading as a font */
  if(hasext(FileName,".FNT")) return(!!LoadFNT(fullPathName));
  /* Try loading as palette */
  if(hasext(FileName,".PAL")) return(!!LoadPAL(fullPathName));
  /* Try loading as cheats */
  if(hasext(FileName,".CHT")) return(!!LoadCHT(fullPathName));
  if(hasext(FileName,".MCF")) return(!!LoadMCF(fullPathName));
...
/*************************************************************/
/* Embedded fallback BIOS, used when the file can't be read off the SD card
 * (missing card, or file not present). See src/cbios_data.c/.h - built from
 * C-BIOS (BSD-style license, see /third_party_licenses/cbios.txt). This
 * keeps stock LoadROM() call sites (which hardcode /sdcard/msx/bios/*.ROM)
 * working unmodified: drop your own dumped MSX.ROM on the SD card to
 * override it, otherwise C-BIOS boots automatically. */
...
byte *LoadROM(const char *Name,int Size,byte *Buf)
{
  ...
  strcpy(fullPath, fullCurrentDir);
  strcat(fullPath, "/bios/");
  ...
  */
  ...
  if(!(F=fopen(Name,"rb")))
  {
    /* No SD card / no file: fall back to the embedded C-BIOS MSX1 main ROM
     * for the one path we ship a substitute for. Everything else (disk
     * BIOS, MSX2 BIOS, FM-PAC, game carts, ...) still requires the SD
     * card, same as stock fMSX. */
    if(Size==32768 && strstr(Name,"MSX.ROM"))
    {
      /* The embedded BIOS is a const array, so it is already mapped and
       * readable in flash. Hand back a pointer to it rather than spending
       * 32kB on a board that has 320kB in total - without this the 16kB
       * of VRAM stops fitting and the machine does not boot at all. The
       * only thing that normally writes to a BIOS image is the cassette
       * patching below, and that is skipped when the image is in flash. */
      if(!Buf) return((byte *)EMBEDDED_BIOS);
      memcpy(Buf,EMBEDDED_BIOS,Size);
      return(Buf);
    }
    return(0);
  }
...
#ifdef HAVE_LOCAL_CART
  /* The cartridge built into the firmware. There is no file to open and,
   * more to the point, no 32kB of RAM to copy it into: the image is a
   * const array, so it is already mapped and readable in flash and the
   * slot can point straight at it. Everything below the MapCart label -
   * the BASIC detection and the whole mirroring layout - then runs
   * unchanged, which is why this jumps into it rather than repeating it. */
  if(!strcmp(FileName,LOCAL_CART_PATH))
  {
    const unsigned char *Img = msx_cart_image();
    int ImgLen = msx_cart_size();

    Len   = ImgLen>>13;                   /* 8kB pages */
    for(Pages=1;Pages<Len;Pages<<=1);
    ROM64 = 0;

    /* A ROM whose size is not a power of two would have to be mirrored,
     * and mirroring means writing into the image - which cannot be done
     * to one in flash. Every cartridge worth having is a power of two. */
    if(Len!=Pages)
    {
      if(Verbose) printf("  Cartridge %c: %dkB is not a power of two, refusing\n",'A'+Slot,Len*8);
      return(0);
    }

    /* Do NOT force this to zero. Anything over 32kB is a MegaROM and the
     * mask is what tells the mapper how many 8kB pages it can switch
     * between; zeroing it turns a 128kB Konami cartridge into a broken
     * 8kB one. */
    ROMMask[Slot] = (Len>4)? (Pages-1):0x00;
    ROMData[Slot] = P = (byte *)Img;
    if(Verbose) printf("  Cartridge %c: %dkB from flash..",'A'+Slot,Len*8);
    goto MapCart;
  }
#endif
