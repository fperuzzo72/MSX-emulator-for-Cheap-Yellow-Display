...
/* A cartridge image built into the firmware, for a board with no room to
 * hold one in RAM. src/local_cart_data.c is generated by
 * tools/embed_rom.py and is gitignored, like the BIOS. LoadCart() takes
 * this path as a name meaning "the one in flash" rather than a file. */
extern const unsigned char local_cart_rom[32768];
...
{ 
  ...
  {
    Len   = sizeof(local_cart_rom)>>13;   /* 8kB pages */
    for(Pages=1;Pages<Len;Pages<<=1);
    ROM64 = 0;
    ROMMask[Slot] = 0x00;
    ROMData[Slot] = P = (byte *)local_cart_rom;
    if(Verbose) printf("  Cartridge %c: %dkB NORMAL ROM from flash..",'A'+Slot,Len*8);
    goto MapCart;
  }
#endif
