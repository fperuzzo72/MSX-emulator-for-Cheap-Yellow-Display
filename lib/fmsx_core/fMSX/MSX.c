...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if(((Mode^NewMode)&MSX_MODEL)
      ...
      * this board, so skipping them costs nothing and saves a 32kB RAM
      * copy of the ROM on a machine that has none to spare. */
     && (MemMap[0][0][0]!=(byte *)local_bios_rom)
#endif
    )
  {
  ...
  /* Upstream floors an MSX1 at 2 pages (32kB), but a real MSX1's TMS9918
   * has 16kB and that is all its modes can address: VDP register 14 is
   * masked with VRAMPages-1, so with one page every access folds into the
   * single page. On a board with no PSRAM those 16kB matter. */
...
byte *LoadROM(const char *Name,int Size,byte *Buf)
{
  ...
  */
  ...
  if(!(F=fopen(Name,"rb")))
  {
    ...
    {
#ifdef HAVE_LOCAL_BIOS
      /* The dumped BIOS is a const array, so it is already mapped and
       * readable in flash. Hand back a pointer to it rather than spending
       * 32kB of a board that has 320kB total. The one page the emulator
       * writes to gets a RAM copy at the call site in ResetMSX(). */
      if(!Buf) return((byte *)local_bios_rom);
      memcpy(Buf,local_bios_rom,Size);
      return(Buf);
#else
      P=Buf? Buf:GetMemory(Size);
      if(!P) return(0);
      memcpy(P,cbios_main_msx1_rom,Size);
      return(P);
#endif
    }
    return(0);
